...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        console.log('📋 Calling apiService.getUserActions()...');
        const actionsResponse = await apiService.getUserActions({ loadMore: false });
        console.log('📋 Actions response:', actionsResponse);
        
        if (actionsResponse.success) {
          console.log('✅ Actions data received:', actionsResponse.data.actions);
          setSuggestedActions(actionsResponse.data.actions);
          setActionsPagination(actionsResponse.data.pagination);
          
          // Calculate and set onboarding completion
          const completion = calculateOnboardingCompletion(actionsResponse.data.actions);
        ...
        console.log('📄 Calling apiService.getUserDocuments()...');
        const documentsResponse = await apiService.getUserDocuments({ loadMore: false });
        console.log('📄 Documents response:', documentsResponse);
        
        if (documentsResponse.success) {
          console.log('✅ Documents data received:', documentsResponse.data.documents);
        ...
        // Calculate completion for fallback data too
  ...
  // Function to fetch filtered actions (moved outside useEffect for reusability)
    ...
    if (userProfile) { // Only fetch if we have initial data
      try {
        ...
        if (response.success) {
          ...
          // Update onboarding completion when actions change
          // Note: We need to get ALL actions for accurate calculation, not just filtered ones
          const allActionsResponse = await apiService.getUserActions({ loadMore: false });
          if (allActionsResponse.success) {
            const completion = calculateOnboardingCompletion(allActionsResponse.data.actions);
            setOnboardingCompletion(completion);
          }
        }
      } catch (error) {
